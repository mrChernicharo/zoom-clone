...
let OtherUsername = ""; // It will hold other user's name
...
navigator.mediaDevices // Webrtc provides a standard api for accessing cameras and microphones connected to the device
  ...
  .then((stream) => {
    ...
    socket.on("user-disconnected", (id) => {
      console.log("user-disconnected", { id, peers });
...
const addVideoStream = (videoEl, stream, name) => {
  ...
  const h1 = document.createElement("h1"); // name display
  ...
  if (totalUsers > 1) {
    ...
    for (let index = 0; index < totalUsers; index++) {
      ...
      document.getElementsByTagName("video")[index].style.width = 100 / totalUsers + "%"; // Set the width of each video to // 👈👈
    }
  }
};
